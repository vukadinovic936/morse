...
A smooth function on a manifold is a Morse function if all critical points are non-degenerate and all the critical points have distinct function values
\end{definition}
...
Also, we assume that it is possible to compute inter-point dinstances between the points in the data.
...
We have implemented the Mapper algorithm and ran it for the sk learn diabetes dataset and standford cars dataset, output are as follows:

\begin{center}
\includegraphics[width=.4\textwidth]{diabetes.png}
\includegraphics[width=.4\textwidth]{cars.png}
\end{center}
Based on the diabetes data we may be able to make a difference between patients who don't need a treatmant, diabetis 1 and diabetis 2. While in the cars dataset it looks like there aren't any great differences between car features.

\textbf{Applications of Reeb Graphs in Computer Graphics}\\
